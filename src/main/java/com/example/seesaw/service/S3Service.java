...
public class S3Service {
    ...
    public List<String> update(Long postId, List<MultipartFile> files) {
        //이미지 삭제 후 재업로드
        delete(postId);
        return upload(files);
    }
    ...
    public void delete(Long itemId){
        List<Image> lastImages = imageRepository.findByPostId(itemId);
        ...
        for(Image lastImage : lastImages){
            if (!"".equals(lastImage.getImageUrl()) && lastImage.getImageUrl() != null) {
                ...
                lastImageUrl = lastImageUrl.replace("https://ceeso.s3.ap-northeast-2.amazonaws.com/", "");
            ...
            imageRepository.deleteById(lastImage.getId());
        }
    }
